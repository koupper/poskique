...
namespace WindowsFormsApplication1.Clases.Datos
{
    class Impresora
    {
        ...
        public string lineasGuio()
        {
            ...
            {
                lineasGuion += "_";//Agregara un guio hasta llegar la numero maximo de caracteres.
            }
            return linea.AppendLine(lineasGuion).ToString(); //Devolvemos la lineaGuion
        }
        public string verTicket()
        {
            StringBuilder verLista = new StringBuilder(linea.ToString());
            verLista.Replace("\u001bp\0\u000f\u0096","");
        ...
        public void AgregaArticulo(string articulo, int cant, decimal precio)
        {
            ...
            if (cant.ToString("#.##", CultureInfo.InvariantCulture).Length <= 5 && precio.ToString("#.##", CultureInfo.InvariantCulture).Length <= 7 )
            {
                ...
                if (articulo.Length > 20)
                {
                    ...
                    nroEspacios = (5 - cant.ToString("#.##", CultureInfo.InvariantCulture).Length);
                    espacios = "";
                    for (int i = 0; i < nroEspacios; i++)
                    {
                        espacios += " ";//Generamos los espacios necesarios para alinear a la derecha
                    }
                    elemento += espacios + cant.ToString("#.##", CultureInfo.InvariantCulture);//agregamos la cantidad con los espacios

                    //Colocar el precio a la derecha.
                    nroEspacios = (7 - precio.ToString("#.##", CultureInfo.InvariantCulture).Length);
                    espacios = "";
                    for (int i = 0; i < nroEspacios; i++)
                    {
                    ...
                    elemento += espacios + "$" + precio.ToString("#.##", CultureInfo.InvariantCulture);//Agregamos el precio a la variable elemento
                ...
                else //Si no es mayor solo agregarlo, sin dar saltos de lineas
                {
                    ...
                    //Colocar la cantidad a la derecha.
                    nroEspacios = (5 - cant.ToString("#.##", CultureInfo.InvariantCulture).Length);// +(20 - elemento.Length);
                    espacios = "";
                    for (int i = 0; i < nroEspacios; i++)
                    {
                        espacios += " ";
                    }
                    elemento += espacios + cant.ToString("#.##", CultureInfo.InvariantCulture);
                    ...
                    elemento += espacios + "$"+precio.ToString("#.##", CultureInfo.InvariantCulture);
        ...
        public void CortaTicket()
        {
          ...
          /*  string d = Convert.ToString((char)100);
            ...
            string GS = Convert.ToString((char)100);
            string ESC = Convert.ToString((char)27);
            string COMMAND = "";
            COMMAND = ESC + "@";
            COMMAND += GS + "V" + (char)1;
            */
            //RawPrinterHelper.SendStringToPrinter(settings.PrinterName, COMMAND);
          //  linea.AppendLine(COMMAND);
            linea.AppendLine("\x1b" + "\x64" + "\x31"); //Avanza 9 renglones, Tambien varian
        ...
        public bool ImprimirTicket(string impresora)
        {
            ...
            impresionCorrecta = RawPrinterHelper.SendStringToPrinter(impresora, linea.ToString() );
            //Imprime texto.
            linea = null;//Al cabar de imprimir limpia la linea de todo el texto agregado.
    ...
    public class RawPrinterHelper
    {
        ...
        public static bool SendStringToPrinter(string szPrinterName, string szString)
        {
            ...
            resultadoImpresion = SendBytesToPrinter(szPrinterName, pBytes, dwCount);
